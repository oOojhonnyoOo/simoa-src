...
class Helper
{
  ...
  public static function encrypt($simple_string, $encryption_key)
  {
  ...
  public static function isPublished($_status)
  {
    return in_array("published", $_status);
  }

  public static function isPreview($_status)
  {
    return in_array("preview", $_status);
  }

  public static function _img($source, $crop)
  {
  ...
  public static function _solrToNumberDate($string)
  {
    $Y = substr($string, 0, 4);
    $m = substr($string, 5, 2);
    $d = substr($string, 8, 2);
  ...
  public static function solrToNumberDate($string)
  {
    return numberDate(self::convertDateToBrDate($string));
  }

  public static function dateAvailable($startDate, $endDate)
  {
    $numberNow = date("YmdHis", time());
  
    if ($endDate !== null && self::solrToNumberDate($endDate) != "" && self::solrToNumberDate($endDate) < $numberNow) {
      return "finished";
    }
  
    if ($startDate !== null && self::solrToNumberDate($startDate) > $numberNow) {
      return "comingSoon";
    }
  ...
  public static function getUrlContent($url, $defaultValue = null, $isJson = true)
  {
   ...
   */
  public static function parseValue($valString)
  {
    if (empty($valString)) {
      return $valString;
    }
   ...
   * @return array||null
   */
  public static function filterData($data, $path = "", $itemFilter = null)
  {
    if (!$path && !$itemFilter) {
      return $data;
    }
    ...
    foreach ($path as $prop) {
      $current = self::_filterItem($current, $prop);
    }
    return self::filterData($current, null, $itemFilter);
  }

  public static function numberDate($string)
  {
    if (empty($string)) {
      return false;
    }
    ...
    $a = explode(" ", $string);
    $date = self::_filterItem($a, 0);
    $hourString  = self::_filterItem($a, 1);
    $hour = ($hourString) ? $hourString : "00:00:00";

    $b = explode("/", $date);
    $d = self::_filterItem($b, 0);
    $m = self::_filterItem($b, 1);
    $Y = self::_filterItem($b, 2);
    $c = explode(":", $hour);
    $H = self::_filterItem($c, 0);
    $i = self::_filterItem($c, 1);
    $s = self::_filterItem($c, 2);
    return (int) "$Y$m$d$H$i$s";
  }

  private static function _filterItem($item, $property)
  {
    if (is_array($item)) {
      if (array_key_exists($property, $item)) {
        return self::_flatArray($item[$property]);
      }
  ...
  private static function _flatArray($array)
  {
    if (!is_array($array)) {
      return $array;
    }
  ...
  public static function isCpf($str)
  {
  ...
  public static function length($prop, $param)
  {
    return (is_string($prop)) ? (strlen($prop) == $param) : (count($prop) == $param);
  }

  public static function convertDateToSolr($date)
  {
    $date = trim($date);
   ...
   * @return string data
   */
  public static function convertDateToBrDate($date_time, $noTime = FALSE)
  {
